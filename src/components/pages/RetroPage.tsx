...
export const RetroPage: React.FC<RetroPageProps> = ({ vehicles }) => {
  ...
  const loadFiles = async () => {
    ...
    try {
      setLoading(true);
      const data = await apiService.getFilesByType('retro', selectedVehicle);
      setFiles(data);
    } catch (error) {
      console.error('Error loading retrofit files:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Retrofitment Summary</h2>
        <p className="text-gray-600">Vehicle Retrofit Documentation</p>
      </div>

      <div className="bg-white rounded-xl shadow-lg p-6">
        {canUpload && (
          <div className="border-b border-gray-200 pb-6 mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Upload Retrofit Summary</h3>
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Retrofit Documents</h3>
          
          {!selectedVehicle ? (
            <p className="text-gray-500 text-center py-8">Please select a vehicle to view retrofit files.</p>
          ) : loading ? (
            ...
            <p className="text-gray-500 text-center py-8">No retrofit files uploaded for this vehicle.</p>
          ) : (
